...
public class ShopManager {
    ...
    public static void main(String[] args) {
        ...
        // Search for products containing ap and search for products containing to, splitted by space
        searchForProduct("ap to");
    }
    
    public static void searchForProduct(String search) {
    	String[] searchPieces = search.split("\\s+");
    	String queryString = "select p from Product as p where lower(p.name) LIKE";
    	
    	for(int i = 0; i < searchPieces.length - 1; i++) {
    		queryString += " '%" + searchPieces[i] + "%'";
    		queryString += " OR LOWER(p.name) LIKE";
    	}
    	
    	queryString += " '%" + searchPieces[searchPieces.length - 1] + "%'";
